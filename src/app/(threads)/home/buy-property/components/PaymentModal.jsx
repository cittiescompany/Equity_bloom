...
import { useBuyPropertyMutation } from '@/api/property';
import { formatCurrency, notifier } from '@/lib/utils';
import { useInvestmentStore, usePropertyStore } from '@/store/Global';
...
const PaymentModal = ({ isOpen, onClose,}) => {
    ...
    const {data,setWalletBanace}=usePropertyStore()
    const {mutateAsync:buyProperty}=useBuyPropertyMutation()

    const handleBuyProperty=async()=>{
      
try {
const resp=  await buyProperty({propertyId:data.selectedProperty._id, amount:data.paymentAmount})
notifier({
  message:resp?.data.data.message?? resp?.data.message?? "Bought Successfully",
  type: "success",
});
console.log(resp)
} catch (e) {
  notifier({
    message:
      e.response?.data?.message ?? e.response?.message ?? "Error Buying this property",
    type: "error",
  });
}
    }
