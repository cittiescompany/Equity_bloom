'use client'
import { useBuyShareMutation } from '@/api/investment';
...
const PaymentModal = ({ isOpen, onClose,}) => {
    ...
    const {mutateAsync:buyShare}=useBuyShareMutation()

    const handleBuyShare=async()=>{
      console.log('investment',data.selectedInvestment);
      
try {
const resp=  await buyShare({investmentId:data.selectedInvestment._id, sharesToBuy:data.noOfShares})
notifier({
...
console.log(resp)
  ...
  notifier({
    message:
      e.response?.data?.message ?? e.response?.data ?? "Error buying shares",
    type: "error",
  });
}
    }
